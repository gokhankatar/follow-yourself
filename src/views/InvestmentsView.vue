<template>
  ...
  <v-container
    v-if="
      $store.state.investments.investmentsList.length == 0 && !$store.state.isShowTitle
    "
  ...
  >
    ...
    >
      <v-row class="d-flex flex-column flex-sm-row justify-space-between">
        ...
        <v-col
          @click="handleInvestment(item, index)"
          class="d-flex justify-start align-center"
          lg="3"
        >
          ...
          <span
            >{{ item.name.toUpperCase() }} <strong>({{ item.genre }})</strong></span
          >
        </v-col>
        ...
        <v-col class="d-flex justify-start align-center" lg="2">
          <v-chip
            ...
            :class="item.status == 'sold' ? 'sold' : 'bought'"
            :prepend-icon="
              item.status == 'sold'
                ? 'fa-solid fa-hand-holding-hand'
                : 'fa-solid fa-hand-holding-dollar'
            "
            :color="item.status == 'sold' ? 'red-lighten-1' : 'success'"
            class="cursor-pointer"
            size="small"
            variant="outlined"
          >
