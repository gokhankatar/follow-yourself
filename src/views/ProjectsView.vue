...
<template>
  ...
  <div class="d-flex justify-center align-center ma-5" v-if="$store.state.isShowTitle">
    <v-icon icon="fa-solid fa-briefcase" color="primary" size="x-large"></v-icon>
  </div>
  ...
  <v-dialog v-model="isAddingProject" transition="dialog-bottom-transition" max-width="400">
    <div class="form-container pa-5">
      <v-form ref="projectForm" @submit.prevent="createProject">
        <v-text-field v-model="projectName" :rules="titleRules" class="text-white" color="primary" label="Project Title"
          variant="outlined" />

        <v-autocomplete v-model="projectStatus" class="mt-2 text-white" :rules="[(v) => !!v || 'state is required!']"
          :items="['ongoing', 'finished']" chips color="primary" item-color="primary" label="Project Status"
          variant="outlined" required />

        <v-btn v-if="!isEditProject" class="mt-3 pa-2" color="success" size="medium" variant="outlined" type="submit"
          block>{{ $t("create") }}</v-btn>

        <v-btn v-if="isEditProject" @click="editProject(selectedItem)" class="mt-3 pa-2" color="primary" size="medium"
          variant="outlined" block>{{ $t("save") }}</v-btn>
        ...
        <v-btn @click="isAddingProject = false" class="mt-3 pa-2" color="warning" size="medium" variant="outlined"
          block>{{ $t("cancel") }}</v-btn>
      </v-form>
    </div>
  </v-dialog>
  ...
  <v-container v-if="$store.state.projects.projectsList.length == 0 && !$store.state.isShowTitle"
    ...
    <v-card v-if="!isEditMode" class="text-caption text-grey">
      <v-row class="px-2 py-1 d-flex justify-space-between">
        <v-col class="d-flex justify-start align-center" lg="1">
          <v-icon @click="isEditMode = true" class="cursor-pointer" icon="fa-solid fa-pen-to-square" color="primary" />
    ...
    <v-card v-if="isEditMode" class="my-4 px-2 py-3 text-body-2 text-primary">
      <v-row class="d-flex justify-space-between px-2 flex-column flex-md-row">
        ...
        <v-col class="d-flex justify-end align-center flex-column flex-md-row" lg="8">
          <v-btn @click="setOngoing" class="mx-2 my-1 my-md-0 w-100 w-md-auto" prepend-icon="fa-solid fa-dumbbell"
            variant="outlined" color="warning">
            {{ $t("set-ongoing") }}
          </v-btn>
          <v-btn class="mx-2 my-1 my-md-0 w-100 w-md-auto" @click="setFinisihed" prepend-icon="fa-solid fa-check"
          ...
          <v-btn class="mx-2 my-1 my-md-0 w-100 w-md-auto" @click="cancelEditMode" prepend-icon="fa-solid fa-xmark"
