<template>
  ...
  >
    <div class="form-container pa-5">
      <v-form ref="projectForm" @submit.prevent="createProject">
        ...
        <v-btn
          v-if="isEditProject"
          @click="editProject(selectedItem)"
          class="mt-3 pa-2"
          color="primary"
          size="medium"
          variant="outlined"
  ...
  <v-container
    v-if="$store.state.projects.projectsList.length == 0 && !$store.state.isShowTitle"
      ...
      <v-row class="d-flex flex-column flex-sm-row justify-space-between">
        ...
        <v-col id="project-status" class="d-flex justify-start align-center" lg="2">
          <v-chip
            ...
            :class="item.status == 'ongoing' ? 'ongoing' : 'success'"
            :prepend-icon="
              item.status == 'ongoing' ? 'fa-solid fa-dumbbell' : 'fa-solid fa-check'
            "
            :color="item.status == 'ongoing' ? 'warning' : 'success'"
            class="cursor-pointer"
            size="small"
            variant="outlined"
          >
