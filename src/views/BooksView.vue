<template>
  ...
  <v-container
    v-if="$store.state.books.booksList.length == 0 && !$store.state.isShowTitle"
  ...
  >
    ...
    <v-card
      v-for="(item, index) of $store.state.books.booksList"
      :key="index"
      ...
      <v-row class="d-flex flex-column flex-sm-row justify-space-between">
        ...
        <v-col class="d-flex justify-start align-center" lg="2">
          <v-chip
            ...
            :prepend-icon="
              item.status == 'will read' ? 'fa-solid fa-dumbbell' : 'fa-solid fa-check'
            "
            :color="item.status == 'will read' ? 'warning' : 'success'"
            variant="outlined"
            class="cursor-pointer"
            size="small"
          >
